...
| `src/content.ts` | 슬롯별 사진·이름, 판정 색, 연락처 | ✅ 12슬롯 대응 |
...
| `src/sync/openai.ts` | LLM 호출 (Vision·슬롯 추출·대화) | ✅ 8초 폴백 |
...
- [x] **12문항 상태머신** — `SLOT_SPEC` 기준, 사진으로 확신한 항목은 한 줄 확인
- [x] **진행률 바** `n / 12`
- [x] **결과 카드 경로 A/B 분리** — 판정 침수심 출처(예측·실측) 함께 표기
...
- [x] **판정 로직 연결** — `src/utils/diagnose.ts` 를 프론트에서 호출 (서버는 침수심 cm 만 제공)
...
- [x] **Structured Output** — 슬롯 추출에 json_schema strict
- [x] **8초 타임아웃 → 버튼 폴백**
